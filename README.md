...
- `docker/` — sshd 설정(`sshd_config`)과 s6 서비스 정의(`s6-rc.d/sshd/`, `s6-rc.d/user-services/`).
...
services:
  hermes:
    image: genichin/js-hermes-agent:20260702-2   # 또는 :latest
...
```sh
# 최초 1회: API 키 설정
docker run -it --rm -v ~/.hermes:/opt/data genichin/js-hermes-agent:20260702-2 setup
